      ...
      <fieldset className="border border-gray-200 rounded-md p-3 sm:p-4">
        ...
        {items.map((item) => (
          ...
          >
            ...
            <button
              type="button"
              className="absolute -right-1 -top-1 sm:static flex items-center justify-center p-2 rounded-full text-red-600 hover:bg-red-100 transition-colors active:bg-red-200 touch-manipulation"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 mt-4 pt-4 border-t border-gray-200">
          <button
            type="button"
            className="inline-flex items-center justify-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200 touch-manipulation"
            onClick={onAdd}
          >
          ...
          <div className="text-center sm:text-right">
            ...
            <span className="ml-2 text-base sm:text-lg font-medium">{currency} {items.reduce((sum, item) => sum + item.amount, 0).toLocaleString()}</span>
          </div>
        </div>
      </fieldset>
    </div>
